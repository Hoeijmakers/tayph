def read_e2ds(inpath,outname,nowave=False,molecfit=False,mode='HARPS',ignore_exp=[]):
    ...
    The user should point this scrip to a folder (located at inpath) that contains
    the pipeline-reduced echelle spectra. The script expects a certain data format,
    depending on the instrument in question.

    The script is designed to accept pipeline products of the HARPS, HARPS-N,
    ESPRESSO and UVES instruments. In the case of HARPS, HARPS-N and ESPRESSO
    these may be downloaded from the archive. UVES is a bit special, because
    the 2D echelle spectra are not a standard pipeline output. Typical use cases
    ...
    barycentric correction (which it doesn't apply) and the airmass, and writes
    these to an ascii table along with the FITS files containing the spectral
    orders.

    A crucial functionality of this script is also that it acts as a wrapper
    ...
    with the 2D fits files. Molecfit is called once in GUI-mode, allowing the user
    to select the relevant fitting regions and parameters, after which it is
    repeated automatically for the entire time series.
    ...
    Advanced Data Products, which include
    ...
    if mode == 'UVES-red' or mode == 'UVES-blue':#IF we are UVES-like
        for i in range(N):
            ...
            if (inpath/Path(filelist[i])).is_dir():
                ...
                berv = np.append(berv,sp.calculateberv(hdr['MJD-OBS'],hdr['HIERARCH ESO TEL GEOLAT'],hdr['HIERARCH ESO TEL GEOLON'],hdr['HIERARCH ESO TEL GEOELEV'],hdr['RA'],hdr['DEC']))
                sci_count += 1
                s1d_count += 1
                e2ds_count += 1
    ...
    if nowave == True and mode not in ['UVES-red','UVES-blue']:
    ...
    if mode in ['UVES-red','UVES-blue']:
        ...
        #entire time series. In normal use cases this should be true.
        diff = wave-wave[0]
